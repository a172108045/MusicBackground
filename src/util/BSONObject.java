...
public class BSONObject {
    ...
    public BSONObject putPointer(String className, String objectId){
        ...
        if(objectId == null){
            bson.remove(className);
        }
        bson.put("__type","Pointer");
        bson.put("className",className);
        bson.put("objectId",objectId);
        return this;
    }

    /**
     * 鐩存帴鍚堝苟value鍊硷紝涓嶅甫鍙屽紩鍙枫€�
     * 涓€鑸笉寤鸿浣跨敤锛岄櫎闈炰綘娓呮鑷繁鍦ㄥ仛浠€涔堛€�
     * @param key
     * @param value
     * @return BSONObject
     */
    public BSONObject putValue(String key, String value){
        ...
        bson.putAll(new BSONObject(BSONString).getMap());
        return this;
    }
    ...
    public String toString(){
        if(isEmpty()){
            ...
            Iterator<Map.Entry<String, Object>> iter = bson.entrySet().iterator();
            while (iter.hasNext()) {
                Map.Entry<String, Object> entry = (Map.Entry<String, Object>) iter.next();
